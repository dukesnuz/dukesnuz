...
if($_SERVER['REQUEST_METHOD'] === 'POST')
   {
	 ...
	 // $q = "INSERT INTO users (username, email, pass, first_name, last_name, date_expires)
		...
		*/
		// below uaing prepared statements chp 12 page 394
		 $q = "INSERT INTO users(username, email, pass, first_name, last_name, date_expires)
			  VALUES(?,?,?,?,?, SUBDATE(NOW(), INTERVAL 1 DAY))";
			 
			//prepare query
			$stmt = mysqli_prepare($dbc,$q);
			//associate PHP variable w/?
			mysqli_stmt_bind_param($stmt, 'sssss', $u, $e, $pass, $fn, $ln);
			
			//execute query and confirm results
			$pass = password_hash($p, PASSWORD_BCRYPT);
			mysqli_stmt_execute($stmt);
			//use below to check for error
