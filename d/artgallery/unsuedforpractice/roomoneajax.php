...
ini_set('display_errors','0');
		...
		//$Find->addSortRule('ModifiedAt', 1, FILEMAKER_SORT_DESCEND);
		$result = $Find->execute();
		 ...
		 $postsArray = $result->getRecords();
			...
			//$records = $result->getFirstRecord(); //$Pictureoftheday = '<img src="'.$record->getField('Imageurl').'"/>';	
	 
	 
	// foreach($postsArray  as $record) {
	  // $record->getField('Imageurl');
	  // $picture = '<img src="'.$record->getField('Imageurl').'"/>';	
	   // echo $picture;
	 //  }
