 ...
 */
var showDescription = function (imageDescriptions, e) {
    "use strict";
    ...
    if (imageDescriptions) {
        ...
        var text = "Visit: " + imageDescriptions.title;
        $('descLink').textContent = "";
        createLink("descLink", link, text);
    } // END if(imageDescriptions){
    addLink(link, e);
};
/***********************
 * 
 * add link to image when user hovers over it
 */
function addLink(link, e) {
    var parent = $(e.target.id).parentElement;
...
window.onload = function () {
    ...
    menu[0].addEventListener('mouseover', function (e) {
        showDescription(imageDescriptions[e.target.id], e);
    }, 'false');
