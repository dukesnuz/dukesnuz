 ...
 * Unit Project 4 - CSCI E - 3 -Introduction to Web Programming Using JavaScript
 * Thank you for checking my work.
 * last updated 12/11111111111111111111111111111/16
 ******************************************************************************************************/
...
(function($) {
	...
	$.fn.playerConnect = function(player) {
		...
		player.onTime(function(evt) {
			...
			//YOUR CODE TO ADD #2 - replace the empty quotes with your code
			...
			*/
			...
			// YOUR CODE TO ADD #3
			// data-start="" data-dur = ""
			// using jQuery each() method to iterate over the data
			$.each(transcriptElements, function(index, value) {
				// get the start and duration times
				// I oringally was using below 2 lines. Just using getAttribute uses less code and seems more efficient
				//var start = parseFloat(transcriptElements[index].attributes["data-start"].nodeValue);
				//var dur = parseFloat(transcriptElements[index].attributes["data-dur"].nodeValue);
				...
				// increment the time by one one hundreds
				time = time + .001;

				// if time greater than or equal to start and time less than or equal to start + duration = total time in span
				if (time >= start && time <= (start + dur)) {
					//add the class to the current span
					$(this).addClass('hilite');
				} else {
					//remove the class from the current span
		...
		return this.click(function(evt) {
			...
			//   I oringinally was using this line
			//   var startSpot = parseFloat(this.attributes["data-start"].nodeValue);
			var startSpot = parseFloat(this.getAttribute("data-start"));
			player.seek(startSpot);
		});
	};
