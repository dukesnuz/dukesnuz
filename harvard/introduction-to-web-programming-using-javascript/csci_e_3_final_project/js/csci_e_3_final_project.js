 ...
 * last updated 12/11111111111111555555555555555555555555/16
 ******************************************************************************************************/
...
function getCities(state) {
	...
	if (window.location.host === "www.dukesnuz.com") {
		...
		ajax.open('GET', './php/csci_3_final_project_retrieve_data.php?state=' + state + '', true);
	} else {
		/***LOCAL***/
...
function editCanvas(value) {
	...
	function rotate() {
		...
		// when green line position is at begining, clear canvas and reset interval
		if (x === 0 && y === 0) {
			clearCanvas();
			setInterval(rotate, intSpeed);
		}
 ...
 * called when one cmplete rotation from rotate function or new states or differnet city is selected
 */
