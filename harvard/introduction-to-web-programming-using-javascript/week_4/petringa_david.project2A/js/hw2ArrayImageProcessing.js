...
**/
/**********A Note**********
 * I read in our textbook on page 199 that:
 * For performance reasons, it is best not to compare i against the array's length with each iteration of the loop.
 * This requires JS to look up the array's length every time. It it better to assign the array's length to another new variable.
 * Then use the new variable for the conditional and use the new variable in the loop's first clause.
 */
function makeBlue(original, output) {
	...
	// set count to array length so array lenght does not need to be calculated
	// with each for loop iteration.
	var count = original.length;
	// Loop through the original array and copy the contents
	...
	//change green value to 0
	for (var i = 1; i < count; i = i + 4) {
		output[i] = 0;
	}
	// Copy the blue value
	for (var i = 2; i < count; i = i + 4) {
		output[i] = original[i];
	}
	// The alpha value will not change
	//END my code
}
...
function makeReverse(original, output) {
	...
	'use strict';
	// Loop through the original array and copy the contents
	// Invert all the colors and alpha using 255-n. n is the element
	// Create a loop inside the loop to change the alpha value back to 255
	// set count equal to the original array length using the length method
	var count = original.length;
...
function makeTransparent(original, output) {
	...
	'use strict';
	// Loop through the array and copy the original
	// Create a loop inside a loop and subtract half the max alpha value(255) from current alpha value
	// Set the original array length to count using the length method
	var count = original.length;
...
function loadComposite(original, secondOne, output) {
	...
	'use strict';
	// Set the original array length to count using the length method
	// Loop through and copy original array and secondOne.
	// Add original and secondOne to output.
	var count = original.length;
